...
export const AiChatbot: React.FC<AiChatbotProps> = ({ currentSchedule = [], onSelectAnime }) => {
  ...
  return (
    <div className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-[280] flex flex-col items-start gap-3 sm:gap-4 max-w-[calc(100vw-2rem)]">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-[calc(100vw-2rem)] sm:w-[390px] max-h-[75vh] sm:max-h-[520px] h-[460px] sm:h-[520px] bg-[#090d16] border border-slate-700/80 rounded-3xl overflow-hidden shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] flex flex-col relative z-[300]"
          >
            {/* Header */}
      ...
      {/* Floating Trigger Button (Positioned Bottom-Left Opposite to Support AniFlow on Bottom-Right) */}
      <button
      ...
      >
        ...
        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-tr from-blue-600 via-indigo-600 to-blue-500 rounded-2xl flex items-center justify-center shadow-3xl border-2 border-white/20 text-white group-hover:rotate-6 transition-transform overflow-hidden relative">
          ...
          <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-[#020617] animate-pulse"></span>
        </div>
      </button>
    </div>
  );
